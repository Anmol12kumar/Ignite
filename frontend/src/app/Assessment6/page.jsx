...
import { level6Questions } from "@/data/level6Questions";
...
const Assessment6 = () => {
    ...
    const handleSubmit = async () => {
        ...
        const q = level6Questions.find((q) => q.id === selectedQ);
        if (!q) return;
    ...
    const activeQuestion = level6Questions.find((q) => q.id === selectedQ);
    const completionPct = Math.round((completedQs.size / level6Questions.length) * 100);
    const isLastQuestion = selectedQ === level6Questions[level6Questions.length - 1].id;
    const allCompleted = completedQs.size === level6Questions.length;
                ...
                <div className="container flex h-14 items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            <span className="text-lg">🚪</span>
                            <span className="font-semibold tracking-tight">Level 6 — Multi-Step Prompt Master</span>
                        </div>
                    </div>
                    {/* Progress bar */}
                    <div className="flex items-center gap-3">
                        <span className="font-mono text-xs text-gray-500">
                            {completedQs.size}/{level6Questions.length} practiced
                        </span>
                ...
                <div className="overflow-y-auto border-r-0" style={{ width: `${dividerPos}%` }}>
                    <div className="p-6">
                        ...
                        <div className="space-y-2">
                            {level6Questions.map((q, index) => {
                                const isUnlocked = index === 0 || completedQs.has(level6Questions[index - 1].id);
                                return (
                                    <button
                                        key={q.id}
                                        onClick={() => {
                                ...
                                <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    {feedback.passed && !isLastQuestion && (
                                        <div className="mt-6 p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/40 text-center flex flex-col items-center">
                                            ...
                                            <Button
                                                onClick={() => {
                                                    const currentIndex = level6Questions.findIndex(q => q.id === selectedQ);
                                                    const nextQ = level6Questions[currentIndex + 1];
                                                    if (nextQ) {
                                    ...
                                    {isLastQuestion && allCompleted && feedback.passed && (
                                        <div className="mt-6 p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/40 text-center flex flex-col items-center">
                                            ...
                                            <p className="text-sm text-emerald-300 mb-4">You have successfully mastered all questions in Level 6.</p>
                                            <Link href="/challenge6" className="w-full">
                                                <Button size="lg" className="w-full text-fuchsia-600 decoration-wavy decoration-fuchsia-800 font-semibold">
                                                    🚀 Ready for the Sixth Challenge
                                                </Button>
                                            </Link>
                                        </div>
                                    )}
