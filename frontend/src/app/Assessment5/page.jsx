...
const Assessment5 = () => {
    ...
    const handleSubmit = async () => {
        ...
        const q = level5Questions.find((q) => q.id === selectedQ);
        if (!q) return;
    ...
    const activeQuestion = level5Questions.find((q) => q.id === selectedQ);
    const completionPct = Math.round((completedQs.size / level5Questions.length) * 100);
    const isLastQuestion = selectedQ === level5Questions[level5Questions.length - 1].id;
    const allCompleted = completedQs.size === level5Questions.length;
                ...
                <div className="container flex h-14 items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            <span className="text-lg">🚪</span>
                            <span className="font-semibold tracking-tight">Level 5 — Creative Prompt Crafter</span>
                        </div>
                    </div>
                    {/* Progress bar */}
                    <div className="flex items-center gap-3">
                        <span className="font-mono text-xs text-gray-500">
                            {completedQs.size}/{level5Questions.length} practiced
                        </span>
                ...
                <div className="overflow-y-auto border-r-0" style={{ width: `${dividerPos}%` }}>
                    <div className="p-6">
                        ...
                        <div className="space-y-2">
                            {level5Questions.map((q, index) => {
                                const isUnlocked = index === 0 || completedQs.has(level5Questions[index - 1].id);
                                return (
                                    <button
                                        key={q.id}
                                        onClick={() => {
                                ...
                                <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    {feedback.passed && !isLastQuestion && (
                                        <div className="mt-6 p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/40 text-center flex flex-col items-center">
                                            ...
                                            <Button
                                                onClick={() => {
                                                    const currentIndex = level5Questions.findIndex(q => q.id === selectedQ);
                                                    const nextQ = level5Questions[currentIndex + 1];
                                                    if (nextQ) {
                                    ...
                                    {isLastQuestion && allCompleted && feedback.passed && (
                                        <div className="mt-6 p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/40 text-center flex flex-col items-center">
                                            ...
                                            <p className="text-sm text-emerald-300 mb-4">You have successfully mastered all questions in Level 5.</p>
                                            <Link href="/challenge5" className="w-full">
                                                <Button size="lg" className="w-full text-fuchsia-600 decoration-wavy decoration-fuchsia-800 font-semibold">
                                                    🚀 Ready for the Fifth Challenge
                                                </Button>
                                            </Link>
                                        </div>
                                    )}
